...
def login():
    ...
    try:
        conn = pyodbc.connect(SQL_CONNECTION_STRING)
        cursor = conn.cursor()
        cursor.execute("SELECT id, username, password_hash, role FROM Users WHERE email = ?", (email,))
        row = cursor.fetchone()
        conn.close()
...
def add_comment(current_user, video_id):
    ...
    try:
        conn = pyodbc.connect(SQL_CONNECTION_STRING)
        cursor = conn.cursor()
        cursor.execute("INSERT INTO Comments (video_id, user_id, content) VALUES (?, ?, ?)",
...
def update_video(current_user, video_id):
    ...
    try:
        ...
        cursor = conn.cursor()
        cursor.execute(
            "UPDATE Videos SET title = ?, description = ? WHERE id = ? AND creator_id = ?",
...
def delete_video(current_user, video_id):
    ...
    try:
        ...
        cursor.execute("SELECT filepath FROM Videos WHERE id = ? AND creator_id = ?", (video_id, current_user['id']))
        row = cursor.fetchone()
...
def upload_video(current_user):
    ...
    try:
        ...
        cursor = conn.cursor()
        cursor.execute(
            "INSERT INTO Videos (title, description, filepath, creator_id, metadata) VALUES (?, ?, ?, ?, ?)",
